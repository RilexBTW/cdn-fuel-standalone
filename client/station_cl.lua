if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    RegisterNetEvent('cdn-fuel:station:client:initiatefuelpickup', function(amountBought, finalReserveAmountAfterPurchase, location)
        if amountBought and finalReserveAmountAfterPurchase and location then
            ...
            if SpawnPickupVehicles() then
                NotifyPlayer(Lang:t("fuel_order_ready"), 'success')
                                                            ...
                                                            if IsControlJustReleased(2, 38) then
                                                                ...
                                                                if distBetweenTruckAndTrailer > 10.0 then
                                                                    ...
                                                                    if not stopNotifyTemp then
                                                                        NotifyPlayer(Lang:t("trailer_too_far"), 'error', 7500)
                                                                    end
                                                                    stopNotifyTemp = true
                                                                    Wait(1000)
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchaselocation', function(data)
        local location = data.location
        local CitizenID = lib.callback('cdn-fuel:getcivID', false, function() end)
        CanOpen = false
        Wait(5)
        lib.callback('cdn-fuel:server:locationpurchased', false, function(result) 
            if result then
        ...
        elseif IsOwned then
            NotifyPlayer(Lang:t("station_already_owned"), 'error', 7500)
        end
    end)
    ...
    RegisterNetEvent('cdn-fuel:stations:client:sellstation', function(data)
        ...
        CanSell = false
        Wait(5)
        lib.callback('cdn-fuel:server:isowner', false, function(result)  
            if result then
                if Config.FuelDebug then print("The Location: "..location.." is owned by ID: "..CitizenID) end
                CanSell = true
            else
                NotifyPlayer(Lang:t("station_not_owner"), 'error', 7500)
        ...
        if CanSell then
            ...
            NotifyPlayer(Lang:t("station_cannot_sell"), 'error', 7500)
        end
    end)
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchasereserves', function(data)
        ...
        Wait(Config.WaitTime)
        if CanOpen then
            local bankmoney = lib.callback('cdn-fuel:getMoney', false, function() end, 'bank')
            ...
            if Config.Ox.Input then
                ...
                if reserves then
                    ...
                    if not reservesAmount then NotifyPlayer(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    Reservebuyamount = tonumber(reservesAmount)
                    if Reservebuyamount < 1 then NotifyPlayer(Lang:t("station_more_than_one"), 'error', 7500) return end
                    if (Reservebuyamount + Currentreserveamount) > Config.MaxFuelReserves then
                        NotifyPlayer(Lang:t("station_reserve_cannot_fit"), "error")
                    else
                        if math.ceil(GlobalTax(Reservebuyamount * Config.FuelReservesPrice) + (Reservebuyamount * Config.FuelReservesPrice)) <= bankmoney then
                            ...
                            NotifyPlayer(Lang:t("not_enough_money_in_bank"), 'error', 7500)
                        end
                    end
                end
            else
                ...
                if reserves then
                    ...
                    if not reserves.amount then NotifyPlayer(Lang:t("station_amount_invalid"), 'error', 7500) return end
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if fuelprice then
                    ...
                    Wait(100)
                    if not fuelPrice then NotifyPlayer(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    NewFuelPrice = tonumber(fuelPrice)
                    if NewFuelPrice < Config.MinimumFuelPrice then NotifyPlayer(Lang:t("station_price_too_low"), 'error', 7500) return end
                    if NewFuelPrice > Config.MaxFuelPrice then
                        NotifyPlayer(Lang:t("station_price_too_high"), "error")
                    else
                ...
                if fuelprice then
                    if Config.FuelDebug then print("Attempting to change fuel price!") end
                    Wait(100)
                    if not fuelprice.price then NotifyPlayer(Lang:t("station_amount_invalid"), 'error', 7500) return end
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if NewName then
                    ...
                    if not NewNameName then NotifyPlayer(Lang:t("station_name_invalid"), 'error', 7500) return end
                    NewName = NewNameName
                    if type(NewName) ~= "string" then NotifyPlayer(Lang:t("station_name_invalid"), 'error') return end
                    if Config.ProfanityList[NewName] then NotifyPlayer(Lang:t("station_name_invalid"), 'error', 7500)
                        -- You can add logs for people that put prohibited words into the name changer if wanted, and here is where you would do it.
                        return
                    end
                    if string.len(NewName) > Config.NameChangeMaxChar then NotifyPlayer(Lang:t("station_name_too_long"), 'error') return end
                    if string.len(NewName) < Config.NameChangeMinChar then NotifyPlayer(Lang:t("station_name_too_short"), 'error') return end
                    Wait(100)
                ...
                if NewName then
                    if Config.FuelDebug then print("Attempting to alter stations name!") end
                    if not NewName.newname then NotifyPlayer(Lang:t("station_name_invalid"), 'error', 7500) return end
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if Withdraw then
                    ...
                    if not WithdrawAmounts then NotifyPlayer(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then NotifyPlayer(Lang:t("station_withdraw_too_little"), 'error', 7500) return end
                    if amount > StationBalance then NotifyPlayer(Lang:t("station_withdraw_too_much"), 'error', 7500) return end
                    WithdrawAmount = tonumber(amount)
                    if (StationBalance - WithdrawAmount) < 0 then
                        NotifyPlayer(Lang:t("station_withdraw_too_much"), 'error', 7500)
                    else
                ...
                if Withdraw then
                    ...
                    if not Withdraw.amount then NotifyPlayer(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then NotifyPlayer(Lang:t("station_withdraw_too_little"), 'error', 7500) return end
                    if amount > StationBalance then NotifyPlayer(Lang:t("station_withdraw_too_much"), 'error', 7500) return end
                    WithdrawAmount = tonumber(amount)
                    if (StationBalance - WithdrawAmount) < 0 then
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if Deposit then
                    ...
                    if not DepositAmounts then NotifyPlayer(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then NotifyPlayer(Lang:t("station_deposit_too_little"), 'error', 7500) return end
                    DepositAmount = tonumber(amount)
                    if (DepositAmount) > bankmoney then
                        NotifyPlayer(Lang:t("station_deposity_too_much"), "error")
                    else
                ...
                if Deposit then
                    ...
                    if not Deposit.amount then NotifyPlayer(Lang:t("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then NotifyPlayer(Lang:t("station_deposit_too_little"), 'error', 7500) return end
                    DepositAmount = tonumber(amount)
                    if (DepositAmount) > bankmoney then
                        NotifyPlayer(Lang:t("station_deposity_too_much"), "error")
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchasemenu', function(location) -- Menu, seen after selecting the purchase this location option.
        ...
        if Config.OneStationPerPerson == true then
            lib.callback('cdn-fuel:server:doesPlayerOwnStation', false, function(result) -- successful port from qb callback to ox ?
                if result then
            ...
            if PlayerOwnsAStation == true then
                NotifyPlayer('You can only buy one station, and you already own one!', 'error')
                return
            end
        end
        ...
        if bankmoney < costofstation then
            NotifyPlayer(Lang:t("not_enough_money_in_bank").." $"..costofstation, 'error', 7500) return
        end
